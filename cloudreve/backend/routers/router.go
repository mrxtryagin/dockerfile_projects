...
func InitMasterRouter() *gin.Engine {
	...
	*/
	...
	// rest接口的分组思想(其实就是拼接path前缀比如这里的v3 指的就是 baseUrl/{v3})
	v3 := r.Group("/api/v3")
	...
	*/
	{
		...
		user := v3.Group("user")
		{
			// 用户登录
			user.POST("session",
				middleware.CaptchaRequired("login_captcha"),
		...
		auth.Use(middleware.AuthRequired())
		{
			...
			file := auth.Group("file", middleware.HashID(hashid.FileID))
			{
				...
				// 创建文件解压缩任务
				file.POST("search/:type/:keywords", controllers.SearchFile)
			}
			...
			aria2 := auth.Group("aria2")
			{
				...
				//重新开始错误的任务 mrx_mod
				aria2.GET("reDownload/:id", middleware.HashID(hashid.TaskID), controllers.ReDownloadTask)
			}
