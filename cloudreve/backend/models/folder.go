...
//UseFolderNameGetFolder 使用文件夹名字获得文件夹
func UseFolderNameGetFolder(folderPath string, userId uint) (bool, *Folder) {
	pathList := util.SplitPath(folderPath)
	if len(pathList) == 0 {
		return false, nil
	}
	...
	for _, folderName := range pathList {
		...
		if folderName == "/" {
			...
			currentFolder, err = GetUserRootById(userId)
			if err != nil {
				return false, nil
			}
		} else {
