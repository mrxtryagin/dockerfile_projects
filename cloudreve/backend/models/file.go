...
// UID为0表示忽略用户，只根据文件ID检索
func GetFilesByKeywords(uid uint, offset int, limit int, keywords ...interface{}) ([]File, error) {
	var (
		files      []File
		result     = DB.Debug()
		conditions string
...
// GetFoldersByKeywords 根据关键字搜索文件,
// UID为0表示忽略用户，只根据文件ID检索
func GetFoldersByKeywords(uid uint, offset int, limit int, keywords ...interface{}) ([]Folder, error) {
	var (
		folders    []Folder
		result     = DB.Debug()
		conditions string
