...
type ItemSearchService struct {
	Type     string `uri:"type" json:"-" `
	Keywords string `uri:"keywords" json:"-" `
	Filetype int    `uri:"-" json:"fileType" `
	ShowNum  int    `uri:"-" json:"showNum" `
	RangeVal [2]int `uri:"-" json:"range"`
}
...
func (service *ItemSearchService) SearchKeywords(c *gin.Context, fs *filesystem.FileSystem, keywords ...interface{}) serializer.Response {
	...
	// 获取子项目
	objects, err := fs.Search(ctx, service.Filetype, service.ShowNum, service.RangeVal, keywords...)
	if err != nil {
