...
//ResumeTaskById 从数据库中恢复指定的id的任务(不管是否错误)
func ResumeTaskById(id interface{}) error {
	task, err := model.GetTasksByID(id)
	if err != nil {
		return err
	}
	// status 变为 进行中 error 清空
	err = task.SetMap(map[string]interface{}{"status": Processing, "error": ""})
	if err != nil {
		return err
	}

	util.Log().Info("即将从数据库中恢复 任务 %d ...", task.ID)

	job, err := GetJobFromModel(task)
	if err != nil {
		util.Log().Warning("无法恢复任务 %d 错误为 %s", task.ID, err)
		return err
	}
	TaskPoll.Submit(job)
	return nil
