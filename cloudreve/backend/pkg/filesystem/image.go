...
// GetThumb 获取文件的缩略图
func (dstfs *FileSystem) GetThumb(ctx context.Context, id uint) (*response.ContentResponse, error) {
	// 根据 ID 查找文件
	err := dstfs.resetFileIDIfNotExist(ctx, id)
	if err != nil || dstfs.FileTarget[0].PicInfo == "" {
		return &response.ContentResponse{
			Redirect: false,
		}, ErrObjectNotExist
	}

	w, h := dstfs.GenerateThumbnailSize(0, 0)
	ctx = context.WithValue(ctx, fsctx.ThumbSizeCtx, [2]uint{w, h})
	ctx = context.WithValue(ctx, fsctx.FileModelCtx, dstfs.FileTarget[0])
	res, err := dstfs.Handler.Thumb(ctx, dstfs.FileTarget[0].SourceName)

	// 本地存储策略出错时重新生成缩略图
	if err != nil && dstfs.Policy.Type == "local" {
		dstfs.GenerateThumbnail(ctx, &dstfs.FileTarget[0])
		res, err = dstfs.Handler.Thumb(ctx, dstfs.FileTarget[0].SourceName)
	}
...
func (dstfs *FileSystem) GenerateThumbnail(ctx context.Context, file *model.File) {
	// 判断是否可以生成缩略图
	...
	// 获取文件数据
	source, err := dstfs.Handler.Get(newCtx, file.SourceName)
	if err != nil {
		return
	}
	defer source.Close()
	...
	// 生成缩略图
	image.GetThumb(dstfs.GenerateThumbnailSize(w, h))
	// 保存到文件
	...
	// 失败时删除缩略图文件
	if err != nil {
		_, _ = dstfs.Handler.Delete(newCtx, []string{file.SourceName + conf.ThumbConfig.FileSuffix})
	}
}

// GenerateThumbnailSize 获取要生成的缩略图的尺寸
func (dstfs *FileSystem) GenerateThumbnailSize(w, h int) (uint, uint) {
