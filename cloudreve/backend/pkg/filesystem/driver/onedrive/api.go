...
var (
	// ChunkSize 服务端中转分片上传分片大小 10MB
	ChunkSize uint64 = conf.OthersConfig.UploadChunkSize * 1024 * 1024
)
...
func (client *Client) UploadChunk(ctx context.Context, uploadURL string, chunk *Chunk) (*UploadSessionResponse, error) {
	...
	if err != nil {
		...
		if chunk.Retried < model.GetIntSetting("onedrive_chunk_retries", 1) {
			chunk.Retried++
			util.Log().Error("分片偏移%d上传失败[%s]，5秒钟后重试", chunk.Offset, err)
			time.Sleep(time.Duration(5) * time.Second)
...
func (client *Client) Upload(ctx context.Context, dst string, size int, file io.Reader) error {
	...
	util.Log().Info("upload to %s,overwrite_type : %s", dst, overwrite)
	//todo: onedrive 上传的返回值不会带名称等信息(比如重命名的话,不会带新的名字) 所以还是只能用replace
	// 小文件，使用简单上传接口上传
	...
	util.Log().Info("file size > 4MB,use chunk_upload,upload to %s start...: totalsize : %s,ChunkSize: %s, chunkTotalNum:%d", dst, util.ConvertSizeToString(float64(size)), util.ConvertSizeToString(float64(ChunkSize)), chunkNum)
	var uploadRes *UploadSessionResponse
	for i := 0; i < chunkNum; i++ {
		...
		default:
			...
			// 上传
			uploadRes, err = client.UploadChunk(ctx, uploadURL, &chunk)
			_end := time.Now().Unix()
			cost := util.If(_end-_start == 0, 0.0001, float64(_end-_start)).(float64)
			_speed, _ := decimal.NewFromFloat(float64(chunkSize) / cost).Round(2).Float64()
			if task != nil {
	...
	//打印uploadRes(这个可以用来修改file)
	//使用onedrive策略来保证file
	util.Log().Info("上传最终的响应体为: %+v", *uploadRes)
	return nil
}
