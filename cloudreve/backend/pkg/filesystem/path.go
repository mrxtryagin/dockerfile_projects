...
// 如果存在就返回目录
func (dstfs *FileSystem) IsPathExist(path string) (bool, *model.Folder) {
	...
	for _, folderName := range pathList {
		...
		if folderName == "/" {
			...
			currentFolder, err = dstfs.User.Root()
			if err != nil {
				return false, nil
			}
		} else {
...
func (dstfs *FileSystem) IsFileExist(fullPath string) (bool, *model.File) {
	basePath := path.Dir(fullPath)
	fileName := path.Base(fullPath)

	// 获得父目录
	exist, parent := dstfs.IsPathExist(basePath)
	if !exist {
		return false, nil
	}
...
func (dstfs *FileSystem) IsChildFileExist(folder *model.Folder, name string) (bool, *model.File) {
