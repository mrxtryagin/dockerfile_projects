...
func (dstfs *FileSystem) Compress(ctx context.Context, folderIDs, fileIDs []uint, isArchive bool) (string, error) {
	// 查找待压缩目录
	folders, err := model.GetFoldersByIDs(folderIDs, dstfs.User.ID)
	if err != nil && len(folderIDs) != 0 {
		return "", ErrDBListObjects
	}

	// 查找待压缩文件
	files, err := model.GetFilesByIDs(fileIDs, dstfs.User.ID)
	...
	for i := 0; i < len(folders); i++ {
		...
		case <-reqContext.Done():
			// 取消压缩请求
			dstfs.cancelCompress(ctx, zipWriter, zipFile, zipFilePath)
			return "", ErrClientCanceled
		default:
			dstfs.doCompress(ctx, nil, &folders[i], zipWriter, isArchive)
		}
	...
	for i := 0; i < len(files); i++ {
		...
		case <-reqContext.Done():
			...
			return "", ErrClientCanceled
		default:
			dstfs.doCompress(ctx, &files[i], nil, zipWriter, isArchive)
		}
	}
...
func (dstfs *FileSystem) cancelCompress(ctx context.Context, zipWriter *zip.Writer, file *os.File, path string) {
...
func (dstfs *FileSystem) doCompress(ctx context.Context, file *model.File, folder *model.Folder, zipWriter *zip.Writer, isArchive bool) {
	// 如果对象是文件
	if file != nil {
		// 切换上传策略
		dstfs.Policy = file.GetPolicy()
		...
		// 获取文件内容
		fileToZip, err := dstfs.Handler.Get(
		...
		if err == nil && len(subFiles) > 0 {
			for i := 0; i < len(subFiles); i++ {
				dstfs.doCompress(ctx, &subFiles[i], nil, zipWriter, isArchive)
			}
		...
		if err == nil && len(subFolders) > 0 {
			for i := 0; i < len(subFolders); i++ {
				dstfs.doCompress(ctx, nil, &subFolders[i], zipWriter, isArchive)
			}
		}
	}
}
...
func (dstfs *FileSystem) Decompress(ctx context.Context, src, dst string) error {
	err := dstfs.ResetFileIfNotExist(ctx, src)
	if err != nil {
		return err
	}
	...
	// 下载压缩文件到临时目录
	fileStream, err := dstfs.Handler.Get(ctx, dstfs.FileTarget[0].SourceName)
	if err != nil {
		return err
	}
	...
	for _, f := range r.File {
		...
		// 如果是目录
		if f.FileInfo().IsDir() {
			dstfs.CreateDirectory(ctx, savePath)
			continue
		}
		...
		case <-worker:
			...
			go func(fileStream io.ReadCloser, size int64) {
				...
				err = dstfs.UploadFromStream(ctx, fileStream, savePath, uint64(size))
				fileStream.Close()
				if err != nil {
