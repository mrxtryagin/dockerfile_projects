...
export default function Folder({ folder, isActive, onIconClick }) {
    ...
    const handleName = ()=>{
        if(keywords !== '' && keywords.indexOf('keywords') !== -1) {
            const keywords_true = keywords.replace("keywords/", '')
            //转义
            const key = keywords_true.trim().replace(/([,.+?:()*[\]^$|{}\\-])/g, '\\$1')
            const re = new RegExp(key,"ig")
            // console.log("keywords_true", keywords_true)
            const newText = folder.name.replace(re,`<span style='background:#ff0;color:red;'>${keywords_true}</span>`)
