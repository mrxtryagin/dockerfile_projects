...
export default function ObjectIcon(props) {
    let path = useSelector((state) => state.navigator.path);
    ...
    // const keywords_true = keywords.replace("keywords/",'').replace("/path/")
    ...
    const handleDoubleClick = (e) => {
        if (props.file.type === "up") {
            return;
        } else if (props.file.type === "dir") {
            e.preventDefault()
            if(keywords !== ''){
                const params = {
                    id: props.file.id,
                    is_dir: props.file.type === "dir" ? 1 : 0
                };
                axios({
                    method: "get",
    ...
    const getFileInfo = (raw) => {
        return "所在目录:" + raw.path;
    ...
    return (
        ...
        >
            {/*<span dangerouslySetInnerHTML = {{ __html: `<strong>${keywords}</strong>` }}/>*/}
            <div
