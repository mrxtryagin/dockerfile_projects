...
class ExplorerCompoment extends Component {
    ...
    contextMenu = (e) => {
        e.preventDefault();
        if (
            this.props.keywords === "" && !pathHelper.isSharePage(this.props.location.pathname)
        ) {
     ...
     getSearchInfo = ()=>{
         ...
         if(key.indexOf("tag") > -1){
             const tags = Auth.GetUser().tags ? Auth.GetUser().tags : []
             const tagId = key.replace('tag/','')
             if(tags){
                 const tag = tags.find(el=>el.id === tagId)
                 resultStr = tag?tag.name:key
             }
         }else{
             resultStr = this.props.keywords.replace("keywords/", '').replace("/path/", "").replace("/internal","")
         }
         const newText =  `搜索<strong><em>${resultStr}</em></strong>结果:    当前条件下共<strong><em>${this.props.dirList.length + this.props.fileList.length}</em></strong>条记录,耗时<strong><em>${this.props.time}</em></strong>s`;
    ...
    render() {
        ...
        return (
            ...
            >
                ...
                {this.props.keywords !== "" && !this.props.loading &&
                !this.props.navigatorError && (<Typography color="textSecondary" variant="h6">
                    {this.getSearchInfo()}
                </Typography>)}
                {((this.props.keywords !== "" &&
                    this.props.dirList.length === 0 &&
