...
class TableRowCompoment extends Component {
    ...
    handleName = ()=>{
        if(this.props.keywords !== '' && this.props.keywords.indexOf('keywords') !== -1) {
            const keywords_true = this.props.keywords.replace("keywords/", '')
            //转义特殊符号
            const key = keywords_true.trim().replace(/([,.+?:()*[\]^$|{}\\-])/g, '\\$1')
            const re = new RegExp(key,"ig")
            // const newText = this.props.file.name.replaceAll(keywords_true,`<span style='background:#ff0;color:red;'>${keywords_true}</span>`)
            const newText = this.props.file.name.replace(re,`<span style='background:#ff0;color:red;'>${keywords_true}</span>`)
            // console.log("newText",newText)
            return (<span dangerouslySetInnerHTML={{ __html: newText }} />)

        }else return this.props.file.name
